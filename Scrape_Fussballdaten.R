# function should get the number of teams playing in a given season
get_number_of_teams_in_season <- function(root_url, league, season){
  ...
  ranking_content <- html_nodes(ranking_page, css = ".ranking-number")
...
# Function returns the results for every match day for a given league
# and season
get_results_for_season <- function(root_url, league, season, number_of_teams){
  ...
  results_whole_season <- data.frame("date" = rep(NA, total_length),
  ...
  for(i in 1:number_of_matchdays){
    ...
    matchday_results <- html_nodes(matchday_results_page, css = ".detils a") %>%
      html_text()
    ...
    matchday_results_clean <- as.data.frame(matrix(matchday_results, ncol = 3,
                                                   byrow = TRUE))
    
    # pre-allocate the needed data frame 
    matches_length <- nrow(matchday_results_clean)
    matchday_results_goals <- data.frame("final_goals_home" = rep(NA, matches_length),
    ...
    for(j in 1:matches_length){
      ...
      matchday_results_game <- substring(matchday_results_clean$V2[j], 
                                         seq(1, nchar(matchday_results_clean$V2[j])-1, 3),
                                         seq(3, nchar(matchday_results_clean$V2[j]), 3)) %>%
        t() %>%
        data.frame() %>%
        separate(col = "X1", into = c("final_goals_home", "final_goals_away"),
                 sep = ":") %>%
        separate(col = "X2", into = c("halftime_goals_home", "halftime_goals_away"),
                 sep = ":")
      
      # store the extracted data into the respective position in the data frame
      matchday_results_goals[j, ] <- c(matchday_results_game$final_goals_home,
