...
post_request_elena <- POST("https://oauth2.elenasport.io/oauth2/token",
...
url_leagues <- paste0(root_endpoint, endpoint, "/", german_league_ids[1])


# Create a data frame to store information on German leagues
german_leagues <- data.frame("league_id" = rep(NA, length(german_league_ids)), 
...
german_frame <- left_join(league_frame, season_frame, by = c("id" = "idLeague"), 
