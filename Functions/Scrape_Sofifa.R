# function to scrape team-based data from sofifa.com
# includes the FIFA game rating for each club and some other information
...
get_team_stats_full <- function(){
  ...
  base_url <- paste0("https://sofifa.com/teams?type=all&lg%5B0%5D=19&lg%5B1%5D=20",
  ...
  for(i in 1:length(fifa_versions)){
    ...
    curr_fifa_url <- paste0(base_url, "&r=", fifa_versions[i], "0001",
                            "&set=true")
    ...
    fifa_version_cur <- ifelse(nchar(trunc(abs(fifa_versions[i]))) == 2,
                               fifa_versions[i], paste0("0", fifa_versions[i]))
    
    fifa_version_prev <- ifelse(nchar(trunc(abs(fifa_versions[i] - 1))) == 2,
    ...
    for(j in 1:length(url_for_date)){
      ...
      club_stats <- read_html(url_for_date[j]) %>%
        ...
        separate(col = Name, into = c("club", "league"),
                 sep = " German") %>%
