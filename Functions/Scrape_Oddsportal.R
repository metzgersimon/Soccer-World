# function returns the past odd results for every game played in a given season
# for a given sport in a given country in a given league
# 
get_past_odd_results <- function(sport, country, league, season, port = 5281L){
  # create a base url with a given sport, country and league
  url_base <- paste0("https://www.oddsportal.com/", sport, "/", country, "/",
                     league)
  ...
  while(TRUE){
    ...
    table_content <- page_html %>% html_nodes("#tournamentTable") %>% 
